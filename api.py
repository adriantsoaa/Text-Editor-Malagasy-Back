...
# ---------- Modèles Pydantic Lemmatisation ----------
class WordAnalysis(BaseModel):
    mot: str
    prefixe: str
    infixe: str
...
# ---------- SYNTHÈSE VOCALE ----------
def fanomana_feo_mg_v15(texte: str) -> str:
    """
    Optimization phonétique pour le malagasy:
    - E -> È (Phonétique 'e' malagasy misokatra)
    - TR -> TCHR (Tahaka ny 'Train')
    - DR -> DJ (Tahaka ny 'Drive')
    - S  -> SS (Mba hijanona ho 'S')
    - O  -> OU | J -> DZ
    """
    # 1. Fanadiovana ny soratra
    t = texte.lower()
    t = re.sub(r'[^\w\s]', ' ', t)
    
    # 2. Fitsipika ho an'ny 'S' (Mba ho 'S' mafy foana)
    t = t.replace('s', 'ss')
    
    # 3. Fanoloana fototra ho an'ny feo (O sy J)
    t = t.replace('o', 'ou').replace('j', 'dz')
    
    # 4. HACKING 'DR' sy 'TR' (Style Anglisy)
    t = t.replace('dr', 'dj')
    t = t.replace('tr', 'tsch')
    
    # 5. NY 'E' MALAGASY (Ampiasaina ny È mba ho misokatra ny feo fa tsy É)
    t = t.replace('e', 'è')
    ...
    for m in mots:
        ...
        # Fanapahana ny 'a' farany amin'ny ka, tra (tchra), na
        if len(m) > 4:
            # Raha mifarana amin'ny 'tra' (izay lasa 'tchrè' izao)
            if m.endswith('tch'):
                m = m[:-1] + 'e'
            
            # Fanapahana ny 'a' farany amin'ny 'ka' sy 'na'
            if m.endswith('kka') or m.endswith('nna'):
                m = m[:-1]
...
async def synthesize(body: PhraseRequest):
    """
    Endpoint pour générer la synthèse vocale d'une phrase en malagasy
    """
    if not body.phrase.strip():
        raise HTTPException(status_code=400, detail="Phrase vide")
    ...
    try:
        ...
        text_hacked = fanomana_feo_mg_v15(body.phrase)
        
        # Génération du fichier audio
        tts = gTTS(text=text_hacked, lang='fr')
        ...
        # Retourner le chemin du fichier (vous pouvez adapter pour servir le fichier directement)
        return AudioResponse(
            url=f"/audio/{os.path.basename(temp_path)}",
...
async def get_audio(filename: str):
    """
    Endpoint pour servir les fichiers audio générés
    """
...
def read_root():
    return {
        "message": "API Malagasy Text Editor avec Synthèse Vocale",
        "endpoints": {
            "correction": "/corriger",
            "sentiment": "/sentiment",
            "lemmatisation": "/lemmatiser",
            "synthese_vocale": "/synthesize",
